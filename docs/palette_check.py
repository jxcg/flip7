...
SWIFT_HEADER = """// Generated by docs/palette_check.py --swift. Do not edit by hand.
//
...
def emit_swift(rows):
    def ramp(name, light_key, dark_key):
        pairs = ",\n".join(
            "    (light: 0x%s, dark: 0x%s)" % (r[light_key][1:], r[dark_key][1:])
            for r in rows
        )
        return "  private static let %s: [(light: UInt32, dark: UInt32)] = [\n%s,\n  ]\n" % (
            name, pairs)

    body = "\n".join([
        ramp("washes", "wash_light", "wash_dark"),
        ramp("inks", "ink_light", "ink_dark"),
        ramp("rails", "rail_light", "rail_dark"),
    ])
    return SWIFT_HEADER + "\n" + body + """}
